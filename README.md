...
Scoping can be used to define styles that are applied only when the current element is in some specific scope. The scoping rule is written using Garden syntax. The style map included in the scoping rule is automatically wrapped in a vector with the name of the autogenerated class name, which creates the scope.
