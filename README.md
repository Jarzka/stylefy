...
If you run multiple stylefy apps on the same domain, they share the same local storage cache as defined by the [Same origin policy](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy). This applies to apps developed on **localhost** domain too. If you want every app to have its own cache, either use a different domain / port for every app, or use [Multi-instance support](#multi-instance).
