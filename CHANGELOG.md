...
- **BREAKING** Removes the possibility to pass `::with-styles` to `use-style` via options map (second argument of `use-style`). This feature has been marked as deprecated since version 1.3.0 (2018). If you have used `::with-styles` in options map, you can easily replace it with `:class`. Notice that passing `::with-styles` via style map (first argument of `use-style`) is still perfectly valid, this change is only related to the options map.
- Adds basic support for Server-Side Rendering. This means that we can now use `use-style` in the backend, when generating Hiccup code. This new feature does not affect how you use stylefy in the frontend.
