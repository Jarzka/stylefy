...
- Style hashing now checks Garden units recursively. This makes sure that if Garden units are used deeply inside a style map, they are converted to CSS before hashing. This makes sure that deeply nestes styles have the same hash value on both frontend and backend.
